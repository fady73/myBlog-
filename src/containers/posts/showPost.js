    ...
    if (this.props.auth.isAuthenticated) {
      ...
      return (
        <div>
          <b>الاجابات</b>
          <div>
            <p>يرجى تسجيل دخول او تسجيل مستخدم جديد لترك اجابتك</p>
          </div>
        </div>
      );
  }
    ...
    if (this.props.auth.uid === post.uid)
      return (
        <div className="form-buttons text-right">
          <Link to={`/question/edit/${post.id}`} className="btn btn-primary">
            <i className="fa fa-edit"></i>تعديل سؤالك
          </Link>
        </div>
      );
  }
...
function validate(values) {
  const errors = {};
  if (!values.comment) errors.comment = "يرجى ادخل هذا الحقل";
